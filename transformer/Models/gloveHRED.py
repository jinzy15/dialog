...
class gloveHRED(HRED):
    ...
    def train(self, session, len_session, criterion, encoder_optimizer, context_optimizer, decoder_optimizer):
        ...
        vec_session = session[0]
        len_session = len(id_session)
        ...
        for i in range(0, len_session - 1):
            input_variable = vec_session[i]
            target_variable = id_session[i + 1]
            last = False
    ...
    def score(self, sentences, answer,max):
        decoded_words = []
        probe = 1
        max_length = max
        ...
        for i, sentence in enumerate(sentences):
            input_variable = self.dataset.vectensorFromSentence(sentence)
            input_length = input_variable.size()[0]
        ...
        # answer_variable = self.dataset.vectensorFromSentence(answer)

        # print(answer)
        id_answer_variable = self.dataset.idtensorFromSentence(answer)
        answer_length = id_answer_variable.size()[0]
        ...
        for di in range(answer_length):
            decoder_output, decoder_hidden, decoder_attention = self.decoder(
                decoder_input, decoder_hidden, encoder_output, encoder_outputs, context_hidden)
            probe = probe * math.exp(float(decoder_output[0][int(id_answer_variable[di])]))
            decoder_input = id_answer_variable[di]  # Teacher forcing
        return probe

    def encodescore(self,sentences):
        context_hidden = self.context.initHidden()
